...
function getPosition(eno){
  ...
  total = getTotal(eno);
  var position = tmparray.sort(function(a, b){return b-a}).indexOf(total)+1; console.log(position);
  return position;


}

function getTotal(eno){
  //console.dir(eno);
  for(var i=0;i<allmarks.length;i++){

    if(allmarks[i].name.toLowerCase() === eno.toLowerCase()){
      var len = allmarks[i].data.length;
      return allmarks[i].data[len-1];
    }
  }
  return "";
